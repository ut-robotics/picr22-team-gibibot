...
*Fusion 360 link:* https://a360.co/3HFLqKh  
...
* Ball detection and depth detection could be improved by using better and more calculations.
* Timer that we used could be made into a function to make the code more compact.
* Line detection is very reliable.
...
=== Thrower design
* Ball grabbing ability is achieved by a servo that rotates a wheel and long ball ramps. The (grey)wheel draws the ball in and stops once the ball reaches the IR sensor. If the robot is ready to throw, the motor will push the ball past the sensor, after that the ball will reach the throwing motor.

image:Images/thrower1.png[] 

* Adjustable throwing angle is achieved by rotating a gear wheel attached to a servo, which rises or lowers the ramp
...
+ Angle adjustment and ball grabbing worked good
